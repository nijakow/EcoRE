...
extern struct Eco_Object*  Eco_Vector_PROXY;
...
bool Eco_VM_Builtin_SetVectorProxy(struct Eco_Fiber* fiber, unsigned int args)
{
    Eco_Any  any;
    ...
    Eco_Any_AssignAny(&any, Eco_Fiber_Peek(fiber));
    if (Eco_Any_IsPointer(&any))
        Eco_Vector_PROXY = Eco_Any_AsPointer(&any);
    else
        return false;
    return true;
}

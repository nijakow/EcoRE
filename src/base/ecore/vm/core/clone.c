...
void Eco_CloneState_Create(struct Eco_CloneState* self, struct Eco_Object* root)
{
                             ...
                             bool forced)
{
    if (Eco_Any_IsPointer(*src)) {
        *dst = Eco_Any_FromPointer(Eco_CloneState_CloneObject(state, Eco_Any_AsPointer(*src), forced));
    } else {
